...
namespace MouseClickCounter
{
    public partial class Program
    {
        ...
        private static void InitializeNotifyIcon()
        {
            ...
            notifyIcon.Icon = new Icon("MouseClickCounter.ico");
        ...
        private static void InitializeAutoSaveTimer()
        {
            ...
            autoSaveTimer.Interval = 5 * 60 * 1000; // 5分鐘
            autoSaveTimer.Tick += (s, e) =>
            {
                foreach (var mouseModel in mouseDataDict.Keys.ToList())
                {
        ...
        private static void DetectMouseModel()
        {
            try
            {
                ...
                foreach (ManagementObject obj in searcher.Get())
                {
                    ...
                    currentMouseModel = "{pnpId} - {name}";
                    if (!string.IsNullOrEmpty(installDate))
                        currentMouseModel += " - 安裝日期: {installDate}";
                    break;
                }
    ...
    public partial class SettingsForm : Form
    {
        ...
        private Button btnSave, btnReset, btnBrowseImage, btnClose;
        private TextBox txtImagePath;
        private ComboBox cmbBrand, cmbModel;
        private TabControl tabControl;
        private TabPage tabRecords, tabSettings;
        public event EventHandler DataUpdated;
        private static List<string> SwitchBrands = new List<string> { "Omron", "Kailh", "Huano", "TTC", "Cherry", "自定義" };
        private static List<string> SwitchModels = new List<string> { "D2FC-F-7N", "GM 8.0", "TTC Dustproof", "自定義" };
        ...
        private void InitializeComponent()
        {
            ...
            tabRecords.Controls.Add(dataGridView);
            Label lblBrand = new Label();
            lblBrand.Text = "微動開關品牌：";
            lblBrand.Location = new Point(30, 40);
            lblBrand.Size = new Size(100, 23);
            cmbBrand = new ComboBox();
            cmbBrand.Location = new Point(140, 40);
            cmbBrand.Size = new Size(200, 23);
            cmbBrand.DropDownStyle = ComboBoxStyle.DropDown;
            cmbBrand.DataSource = SwitchBrands.ToList();
            Label lblModel = new Label();
            lblModel.Text = "微動開關型號：";
            lblModel.Location = new Point(30, 80);
            lblModel.Size = new Size(100, 23);
            cmbModel = new ComboBox();
            cmbModel.Location = new Point(140, 80);
            cmbModel.Size = new Size(200, 23);
            cmbModel.DropDownStyle = ComboBoxStyle.DropDown;
            cmbModel.DataSource = SwitchModels.ToList();
            Label lblImage = new Label();
            lblImage.Text = "微動開關圖片：";
            lblImage.Location = new Point(30, 120);
            lblImage.Size = new Size(100, 23);
            txtImagePath = new TextBox();
            txtImagePath.Location = new Point(140, 120);
            txtImagePath.Size = new Size(400, 23);
            btnBrowseImage = new Button();
            btnBrowseImage.Text = "瀏覽";
            btnBrowseImage.Location = new Point(550, 120);
        ...
        private void LoadData()
        {
            ...
            if (dataGridView.Rows.Count > 0)
            {
                ...
                {
                    if (dataGridView.Rows[i].Cells["MouseModel"].Value.ToString() == currentMouseModel)
                    {
                        index = i;
                        break;
                    }
        ...
        private void UpdateSettingsTabFromSelected()
        {
            if (dataGridView.SelectedRows.Count > 0)
            {
                ...
                {
                    MouseData data = mouseDataDict[mouseModel];
                    cmbBrand.SelectedItem = SwitchBrands.Contains(data.SwitchBrand) ? data.SwitchBrand : "自定義";
                    cmbBrand.Text = data.SwitchBrand;
                    cmbModel.SelectedItem = SwitchModels.Contains(data.SwitchModel) ? data.SwitchModel : "自定義";
                    cmbModel.Text = data.SwitchModel;
                    txtImagePath.Text = (data.ImagePath != null) ? data.ImagePath : "";
                }
            }
        }
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (dataGridView.SelectedRows.Count > 0)
                {
                    ...
                    if (mouseDataDict.ContainsKey(mouseModel))
                    {
                        mouseDataDict[mouseModel].SwitchBrand = cmbBrand.Text;
                        mouseDataDict[mouseModel].SwitchModel = cmbModel.Text;
                        mouseDataDict[mouseModel].ImagePath = txtImagePath.Text;
                    }
                }
